...
$historyRequests = $history_requests ?? [];
...
<main class="space-y-8">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <?php if (!in_array($req['status'], ['completed', 'cancelled'])): ?>
                                        <div class="space-y-4">
                                            ...
                                            <form method="POST"
                                                  class="flex flex-col gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <h2 class="text-xs text-gray-700">
                                            <?= htmlspecialchars($h['user_name'] ?? $h['user_name'] ?? '') ?>
                                        </h2>
                                        <div class="mt-1 text-sm text-gray-500">
                                                <span>
                                                    <?= htmlspecialchars($h['vehicle_brand'] ?? '') ?>
                                                    <?= htmlspecialchars($h['vehicle_model'] ?? '') ?>
                                                    (<?= htmlspecialchars($h['vehicle_year'] ?? '') ?>)
                                                </span>
                                            <?php if (isset($h['scheduled_datetime'])): ?>
                                                <span class="mx-1">•</span>
                                                <time datetime="<?= $h['scheduled_datetime'] ?>"><?= date('d M Y', strtotime($h['scheduled_datetime'])) ?></time>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
    ...
    <?php if (empty($activeRequests) && empty($historyRequests)): ?>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 px-6 py-12 text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary-100 text-primary-600 ">
                <i data-lucide="car" class="h-8 w-8"></i>
            </div>
            <h3 class="text-xl font-medium text-gray-900 mb-2">Belum ada permintaan servis</h3>
            <p class="text-sm text-gray-500 ">Mulai dengan membuat permintaan servis baru untuk kendaraan
                Anda</p>
            <a href="#"
