...
// Current path (to handle active sidebar links)
...
<div x-data="{ sidebarOpen: false }" class="flex h-screen overflow-hidden">
    ...
    <div
            ...
            class="fixed inset-0 bg-gray-600 bg-opacity-50 z-30 md:hidden transition-opacity duration-300"
            x-cloak
    ></div>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="flex items-center justify-between h-16 border-b px-6">
            <span class="text-lg font-semibold text-primary-600">Auto Service</span>
        ...
        <nav class="px-4 py-6 space-y-2">
            <?php foreach ($menu as $item): ?>
                <?php
                $active = isActive($item['link'], $currentPath);
                $linkClasses = $active
                ...
                <a
                        ...
                        class="flex items-center gap-3 px-3 py-2 text-sm rounded-lg <?= $linkClasses ?>"
                >
        ...
        <main class="flex-1 p-4 space-y-6 overflow-y-auto">
            <?php if (isset($pageHeader)): ?>
                <div class="flex items-center justify-between mb-6">
                    <h1 class="text-2xl font-semibold text-gray-900"><?= htmlspecialchars($pageHeader) ?></h1>
                    <?php if (isset($headerButton)) echo $headerButton; ?>
                </div>
            <?php endif; ?>
            {{content}}
        </main>
